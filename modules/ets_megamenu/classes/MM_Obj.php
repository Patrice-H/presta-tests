...
class MM_Obj extends ObjectModel
{
        ...
        if($configs)
        {
            ...
            {                
                ...
                $confFields = array(
                    ...
                    'img_del_link' => $this->id && isset($config['type']) && $config['type']=='file' && $this->$key!='' && @file_exists(dirname(__FILE__).'/../views/img/upload/'.$this->$key) ? $helper->module->baseAdminUrl().'&deleteimage='.$key.'&itemId='.$this->id.'&mm_object=MM_'.Tools::ucfirst($fields_form['form']['name']) : false, 
                );
        ...
        else
        {
            ...
            if(isset($configs['sort_order']) && $configs['sort_order'])
            {
                Db::getInstance()->execute("
                    UPDATE "._DB_PREFIX_."ets_mm_".$this->fields['form']['name']."
            ...
            if($this->id && isset($this->fields['form']['connect_to']) && $this->fields['form']['connect_to'] 
                && ($subs = Db::getInstance()->executeS("SELECT id_".$this->fields['form']['connect_to']." FROM "._DB_PREFIX_."ets_mm_".pSQL($this->fields['form']['connect_to']). " WHERE id_".$this->fields['form']['name']."=".(int)$this->id)))
            {
                foreach($subs as $sub)
                {
    ...
    {  
       return ($max = Db::getInstance()->getValue("SELECT max(".pSQL($key).") FROM "._DB_PREFIX_."ets_mm_".$this->fields['form']['name'].($group && ($groupval > 0) ? " WHERE ".pSQL($group)."=".(int)$groupval : ''))) ? (int)$max : 0;
    }   
    ...
    public function saveData()
    {
        ...
        if($this->fields['form']['name']=='menu')
        {
            ...
            if((int)Tools::getValue('sub_menu_max_width')<10 || (int)Tools::getValue('sub_menu_max_width') > 100)
                $errors[] = Ets_megamenu::$trans['sub_menu_width_invalid'];
            ...
            if($configs)
            {
                foreach($configs as $key => $config)
                {
                    ...
                    elseif($config['type']=='categories' && isset($config['tree']['use_checkbox']) && $config['tree']['use_checkbox'] || $config['type']=='checkbox')
                        $this->$key = Tools::getValue($key) ? implode(',',Tools::getValue($key)): '';
                    else
    ...
    public function duplicateItem($id_parent = false)
    {
        ...
        if($this->add())
        {
            ...
            if(isset($this->fields['form']['connect_to']) && $this->fields['form']['connect_to']
                && ($subs = Db::getInstance()->executeS("SELECT id_".$this->fields['form']['connect_to']." FROM "._DB_PREFIX_."ets_mm_".pSQL($this->fields['form']['connect_to']). " WHERE id_".$this->fields['form']['name']."=".(int)$oldId)))
            {
                foreach($subs as $sub)
                {
                    $className = 'MM_'.Tools::ucfirst(Tools::strtolower($this->fields['form']['connect_to']));
