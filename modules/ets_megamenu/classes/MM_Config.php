        ...
        if($configs)
        {
            foreach($configs as $key => $config)
            {
                $confFields = array(
                    ...
                    'img_del_link' => isset($config['type']) && $config['type']=='file' && Configuration::get($key)!='' && @file_exists(dirname(__FILE__).'/../views/img/upload/'.Configuration::get($key)) ? $helper->module->baseAdminUrl().'&deleteimage='.$key.'&itemId='.$this->id.'&mm_object=MM_'.Tools::ucfirst($fields_form['form']['name']) : false,                     
                );
                if(isset($config['tree']) && $config['tree'])
                {
                    ...
                    if(isset($config['tree']['use_checkbox']) && $config['tree']['use_checkbox'])
                        $confFields['tree']['selected_categories'] = $val ? explode(',',$val):array();
                    else
                        $confFields['tree']['selected_categories'] = array($val);
                }                    
